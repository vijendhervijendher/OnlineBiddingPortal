...
<html lang="en">
  <head>
    ...
    <title>Posted Bids Details</title>
          ...
          <h2 class="sub-header">PRODUCT DETAILS</h2>
            ...
            <table class="table table-striped,sortable">
              <thead>
                <tr>
                  <th>Item id</th>
                  <th><%= header.pname_key %></th>
                  <th><%= header.Base_key %></th>
                  <th><%= "status" %></th>
                  <th>Delete Row </th>
              ...
              <tbody>
              	<%
              		...
              		for(int i =0; i < product_array.size();i++){
    					String res = (String)product_array.get(i);
    					JSONObject obj = (JSONObject) parser.parse(res);
             			url += "<tr>";
              			url += "<td>"+"<a href="+header.bidsurl+"?id="+(String)obj.get(header.itemid_key)+">"+(String)obj.get(header.itemid_key)+"</a>"+"</td>";		
              			url += "<td>"+(String)obj.get(header.pname_key)+"</td>";
              			url += "<td>"+(String)obj.get(header.Base_key)+"</td>";
              			
              			if(("0").equals((String)obj.get("ended"))){
              				url += "<td>"+ "In Progress"+"</td>";	
              				url +="<td>"+"<button value=\"drop\"  id=\""+(String)obj.get(header.itemid_key)+"\">Drop</button>"+"</td>";
              			}else{
              				url += "<td>"+ "Sale Ended"+"</td>";
              			}
